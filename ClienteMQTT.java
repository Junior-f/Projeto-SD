    ...
    public synchronized void publicar(String topic, byte[] payload, int qos, boolean retained) {
        try {
            if (client.isConnected()) {
                ...
                System.out.println(String.format("mensagem: %s publicado. %dB", topic, payload.length));
            } else {
                System.out.println("Cliente desconectado, não foi possível publicar a mensagem " + topic);
            }
        } catch (MqttException ex) {
